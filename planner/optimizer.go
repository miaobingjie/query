...
type Optimizer interface {
	Initialize(builder Builder, limit, offset expression.Expression, order *algebra.Order, distinct algebra.ResultTerms)
	OptimizeQueryBlock(node algebra.Node) (
...
func (this *builder) AddOuterOnclause(onclause expression.Expression,
	baseKeyspaces map[string]*base.BaseKeyspace, keyspaceNames map[string]string) error {
	_, err := ClassifyExpr(onclause, baseKeyspaces, keyspaceNames, true, true, true,
...
func (this *builder) buildJoinOp(join *algebra.AnsiJoin, nest *algebra.AnsiNest,
	...
	if join != nil {
		if hash {
			joinFilter, selec, err := this.getFilter(join.Alias(), join.Onclause())
			if err != nil {
		...
		if hash {
			joinFilter, selec, err := this.getFilter(nest.Alias(), nest.Onclause())
			if err != nil {
				return nil, nil, nil, nil, err
			}
